...
export default function LoginPage() {
  ...
  <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
    <div className="text-center">
      ...
      <div className="mx-auto h-12 w-40 bg-[#05A4BA] rounded-2xl flex items-center justify-center mb-4 shadow-lg shadow-cyan-100/50">
        <h2 className="text-white text-2xl font-bold tracking-tight">KU Mentor</h2>
      </div>
      <p className="mt-2 text-sm text-slate-500 font-medium">Твой персональный ИИ-секретарь</p>
    </div>

    {/* Форма */}
    <form className="mt-8 space-y-4" onSubmit={handleLogin}>
      {error && (
        <div className="bg-red-50 border border-red-100 text-red-600 px-4 py-3 rounded-xl text-sm font-medium text-center animate-shake">
          {error}
        </div>
      )}
      ...
      <div className="space-y-4">
        <div>
          ...
          <input 
            ...
            className="block w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#05A4BA] focus:bg-white transition-all placeholder:text-slate-300"
            placeholder="example@mail.com"
          />
        </div>

        <div>
          <label className="block text-sm font-semibold text-slate-700 ml-1 mb-1">Пароль</label>
          <input
            type="password"
            required
            value={password}
    ...
    <div className="relative my-6">
      ...
      <div className="relative flex justify-center text-sm">
        <span className="px-2 bg-white text-slate-400 font-medium">или</span>
      </div>
    </div>
